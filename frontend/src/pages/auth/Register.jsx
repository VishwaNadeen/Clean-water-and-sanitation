...
function CountryFlag({ src, alt }) {
  ...
  return (
    <img src={src} alt={alt} className="h-5 w-7 rounded border border-sky-100 object-cover" loading="lazy" />
  );
}
...
const sectionHeadingClass =
  "flex items-center gap-2 text-[11px] font-bold text-sky-400 uppercase tracking-widest";
...
function DropdownList({ items, loading, onSelect, searchValue, onSearchChange, placeholder = "Search…", renderItem }) {
  return (
        ...
        {loading ? (
          ...
          items.map((item, i) => (
            <button key={i} type="button" onClick={() => onSelect(item)}
              ...
              {renderItem ? renderItem(item) : <span className="text-sm text-sky-800">{item}</span>}
            </button>
          ))
        )}
      </div>
...
export default function Register() {
  ...
  const [form, setForm] = useState({
    ...
    country: "", provinceState: "", district: "", city: "",
    profilePhoto: null,
  });
  ...
  useEffect(() => {
    let mounted = true;
    if (!form.country) { setStates([]); setDistricts([]); setCities([]); return () => { mounted = false; }; }
    (async () => {
      try {
  ...
  useEffect(() => {
    let mounted = true;
    if (!form.country || !form.provinceState) { setDistricts([]); setCities([]); return () => { mounted = false; }; }
    (async () => {
      try {
        if (form.country === SRI_LANKA_NAME) {
          const [districtData, cityData] = await Promise.all([
            ...
            form.district ? fetchWorldCitiesByDistrict(form.country, form.provinceState, form.district) : Promise.resolve([]),
          ]);
          if (mounted) { setDistricts(Array.isArray(districtData) ? districtData : []); setCities(Array.isArray(cityData) ? cityData : []); }
          return;
        }
        const data = await fetchWorldCities(form.country, form.provinceState);
        if (mounted) { setDistricts([]); setCities(Array.isArray(data) ? data : []); }
      } catch {
        if (mounted) { setDistricts([]); setCities([]); }
      }
    })();
    return () => { mounted = false; };
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      if (countryFieldDropdownRef.current && !countryFieldDropdownRef.current.contains(e.target)) setCountryFieldOpen(false);
      if (provinceDropdownRef.current && !provinceDropdownRef.current.contains(e.target)) setProvinceOpen(false);
      if (districtDropdownRef.current && !districtDropdownRef.current.contains(e.target)) setDistrictOpen(false);
      if (cityDropdownRef.current && !cityDropdownRef.current.contains(e.target)) setCityOpen(false);
    };
  ...
  const filteredCountries = useMemo(() => {
    ...
    return countries.filter((c) => c.name.toLowerCase().includes(kw) || c.dialCode.includes(kw) || c.code.toLowerCase().includes(kw));
  ...
  const filteredWorldCountries = useMemo(() => {
    ...
    return kw ? worldCountries.filter((c) => String(c).toLowerCase().includes(kw)) : worldCountries;
  ...
  const filteredDistricts = useMemo(() => {
    ...
    return kw ? districts.filter((d) => String(d).toLowerCase().includes(kw)) : districts;
  ...
  const handleCountrySelect = (c) => { setForm((p) => ({ ...p, countryCode: c.dialCode })); setCountryOpen(false); setCountrySearch(""); };
  const handleGenderSelect = (v) => { setForm((p) => ({ ...p, gender: v })); setGenderOpen(false); };
  const handleWorldCountrySelect = (v) => { setForm((p) => ({ ...p, country: v, provinceState: "", district: "", city: "" })); setCountryFieldOpen(false); setCountryFieldSearch(""); };
  const handleProvinceSelect = (v) => { setForm((p) => ({ ...p, provinceState: v, district: "", city: "" })); setProvinceOpen(false); setProvinceSearch(""); };
  const handleDistrictSelect = (v) => { setForm((p) => ({ ...p, district: v, city: "" })); setDistrictOpen(false); setDistrictSearch(""); };
  const handleCitySelect = (v) => { setForm((p) => ({ ...p, city: v })); setCityOpen(false); setCitySearch(""); };
  ...
  const validateForm = () => {
    ...
    if (form.password !== form.confirmPassword) { setPasswordError("Passwords do not match."); return "Passwords do not match."; }
    setPasswordError("");
    return "";
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.addressLine1.trim()) payload.append("addressLine1", form.addressLine1.trim());
      if (form.addressLine2.trim()) payload.append("addressLine2", form.addressLine2.trim());
      if (form.addressLine3.trim()) payload.append("addressLine3", form.addressLine3.trim());
      if (form.country.trim()) payload.append("country", form.country.trim());
      if (form.provinceState.trim()) payload.append("provinceState", form.provinceState.trim());
      if (form.district.trim()) payload.append("district", form.district.trim());
      if (form.city.trim()) payload.append("city", form.city.trim());
      if (form.profilePhoto instanceof File) payload.append("profilePhoto", form.profilePhoto);

      const res = await fetch(`${API_BASE_URL}/users`, { method: "POST", body: payload });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || "Registration failed");
      navigate("/auth/verify-email", { state: { email: form.email.trim().toLowerCase() } });
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-50 via-sky-100 to-blue-200 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-2xl">
        ...
        <div className="mb-6 flex flex-col items-center text-center">
          <div className="flex items-center justify-center w-16 h-16 rounded-2xl bg-sky-100/80 border border-sky-200 mb-4">
            <RestroomIcon />
          </div>
          <h1 className="text-2xl font-bold text-sky-900">Create your profile</h1>
        ...
        {/* ── Glass card ── */}
        <div className="bg-white/75 backdrop-blur-xl border border-sky-200/70 rounded-3xl shadow-xl shadow-sky-100/40 overflow-hidden">

          {/* Progress indicator */}
          <div className="h-1 w-full bg-sky-100">
            <div className="h-1 bg-gradient-to-r from-sky-400 to-blue-500 w-1/3 rounded-full transition-all duration-500" />
          </div>
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <div className="px-8 pt-8 pb-6">
              <div className={`${sectionHeadingClass} mb-5`}>
                <span className="flex items-center justify-center w-5 h-5 rounded-full bg-sky-100 text-[10px] font-bold text-sky-600 border border-sky-200">1</span>
              ...
              <div className="mb-5 flex items-center gap-4">
                <div className="relative flex-shrink-0">
                  <div className="w-16 h-16 rounded-full border-2 border-sky-200 bg-sky-50/80 overflow-hidden flex items-center justify-center">
                    {photoPreview ? (
                      <img src={photoPreview} alt="Profile preview" className="w-full h-full object-cover" />
                    ) : (
                      <span className="text-sky-300"><PhotoIcon /></span>
                    )}
                  </div>
                  <button type="button" onClick={() => photoInputRef.current?.click()}
                    className="absolute -bottom-1 -right-1 flex items-center justify-center w-6 h-6 rounded-full bg-sky-500 border-2 border-white text-white hover:bg-sky-600 transition">
                  ...
                  <input ref={photoInputRef} type="file" accept="image/*" onChange={handleProfilePhotoChange} className="hidden" />
                </div>
                <div>
                  <p className="text-sm font-semibold text-sky-900">Profile photo</p>
                  <p className="text-xs text-sky-400 mt-0.5">
                    {form.profilePhoto?.name
                      ? <span className="text-sky-600 font-medium">{form.profilePhoto.name}</span>
                      : "JPG, PNG or GIF — optional"}
                  </p>
                  <button type="button" onClick={() => photoInputRef.current?.click()}
                    className="mt-1.5 text-xs font-semibold text-sky-600 hover:text-sky-800 underline underline-offset-2 transition">
                ...
                <div>
                  ...
                  <div className="relative">
                    <input type="text" name="firstName" value={form.firstName} onChange={handleChange}
                      required placeholder="First name" className={fieldClass} />
                ...
                <div>
                  ...
                  <div className="relative">
                    <input type="text" name="lastName" value={form.lastName} onChange={handleChange}
                      required placeholder="Last name" className={fieldClass} />
                    <FieldIcon><UserIcon /></FieldIcon>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4">
                ...
                <div className="relative">
                  <input type="email" name="email" value={form.email} onChange={handleChange}
                    required placeholder="you@example.com" className={fieldClass} />
              ...
              <div className="mt-4">
                ...
                <div className="grid gap-3 grid-cols-[160px_1fr]">
                  <div ref={countryDropdownRef} className="relative">
                    <button type="button" onClick={() => setCountryOpen((p) => !p)} className={dropdownBtnClass}>
                  ...
                  <div className="relative">
                    <input type="tel" name="phone" value={form.phone} onChange={handlePhoneChange}
                      required placeholder="Phone number" inputMode="numeric" pattern="[0-9]*" className={fieldClass} />
                  ...
                  <div ref={genderDropdownRef} className="relative">
                    <button type="button" onClick={() => setGenderOpen((p) => !p)}
                      ...
                      <span className="flex items-center gap-2">
                        <span className="flex items-center justify-center w-7 h-7 rounded-full bg-sky-100/80 text-sky-500"><GenderIcon /></span>
                      ...
                      <div className="absolute z-20 mt-2 w-full overflow-hidden rounded-2xl border border-sky-200 bg-white shadow-xl shadow-sky-100/40">
                        <div className="p-2">
                          {genderOptions.map((opt) => {
                            ...
                            return (
                              <button key={opt.value} type="button" onClick={() => handleGenderSelect(opt.value)}
                                className={`flex w-full items-center justify-between rounded-xl px-4 py-3 text-sm transition ${
                                  active ? "bg-gradient-to-r from-sky-400 to-blue-500 font-semibold text-white" : "text-sky-800 hover:bg-sky-50"
                                }`}>
                                <span>{opt.label}</span>
                                {active && (
            ...
            {/* ── Divider ── */}
            <div className="h-px bg-sky-50 mx-8" />

            {/* ── SECTION 2: Address ── */}
            <div className="px-8 pt-6 pb-6">
              <div className={`${sectionHeadingClass} mb-5`}>
                <span className="flex items-center justify-center w-5 h-5 rounded-full bg-sky-100 text-[10px] font-bold text-sky-600 border border-sky-200">2</span>
                Address <span className="text-sky-300 font-normal normal-case tracking-normal text-[10px]">— optional</span>
                <div className="flex-1 h-px bg-sky-100" />
              </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" name="addressLine1" value={form.addressLine1} onChange={handleChange}
                    placeholder="Street address" className="w-full rounded-xl border border-sky-200 bg-sky-50/70 px-4 py-3 text-sm text-sky-900 placeholder:text-sky-300 outline-none transition focus:border-sky-400 focus:bg-white focus:ring-4 focus:ring-sky-100" />
                </div>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <input type="text" name="addressLine2" value={form.addressLine2} onChange={handleChange}
                      placeholder="Apt, suite, etc." className="w-full rounded-xl border border-sky-200 bg-sky-50/70 px-4 py-3 text-sm text-sky-900 placeholder:text-sky-300 outline-none transition focus:border-sky-400 focus:bg-white focus:ring-4 focus:ring-sky-100" />
                  </div>
                  <div>
                    <label className={labelClass}>Address line 3</label>
                    <input type="text" name="addressLine3" value={form.addressLine3} onChange={handleChange}
                      placeholder="Building, floor, etc." className="w-full rounded-xl border border-sky-200 bg-sky-50/70 px-4 py-3 text-sm text-sky-900 placeholder:text-sky-300 outline-none transition focus:border-sky-400 focus:bg-white focus:ring-4 focus:ring-sky-100" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div ref={countryFieldDropdownRef} className="relative">
                    <button type="button" onClick={() => setCountryFieldOpen((p) => !p)}
                      className={`${dropdownBtnClass} ${countryFieldOpen ? "border-sky-400 bg-white ring-4 ring-sky-100" : ""}`}>
                      <span className={form.country ? "text-sky-900" : "text-sky-300"}>{form.country || "Select country"}</span>
                    </button>
                    <FieldIcon><ChevronIcon open={countryFieldOpen} /></FieldIcon>
                    {countryFieldOpen && (
                      <DropdownList items={filteredWorldCountries} loading={worldCountriesLoading}
                        searchValue={countryFieldSearch} onSearchChange={setCountryFieldSearch}
                        placeholder="Search country…" onSelect={handleWorldCountrySelect} />
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <div ref={provinceDropdownRef} className="relative">
                      <button type="button" onClick={() => form.country && setProvinceOpen((p) => !p)}
                        className={`${dropdownBtnClass} ${provinceOpen ? "border-sky-400 bg-white ring-4 ring-sky-100" : ""} ${!form.country ? "cursor-not-allowed opacity-50" : ""}`}>
                        <span className={form.provinceState ? "text-sky-900" : "text-sky-300"}>{form.provinceState || "Select province"}</span>
                      </button>
                      <FieldIcon><ChevronIcon open={provinceOpen} /></FieldIcon>
                      {provinceOpen && form.country && (
                        <DropdownList items={filteredStates} loading={false}
                          searchValue={provinceSearch} onSearchChange={setProvinceSearch}
                          placeholder="Search province…" onSelect={handleProvinceSelect} />
                      )}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    {form.country === SRI_LANKA_NAME ? (
                      <div ref={districtDropdownRef} className="relative">
                        <button type="button" onClick={() => form.provinceState && setDistrictOpen((p) => !p)}
                          className={`${dropdownBtnClass} ${districtOpen ? "border-sky-400 bg-white ring-4 ring-sky-100" : ""} ${!form.provinceState ? "cursor-not-allowed opacity-50" : ""}`}>
                          <span className={form.district ? "text-sky-900" : "text-sky-300"}>{form.district || "Select district"}</span>
                        </button>
                        <FieldIcon><ChevronIcon open={districtOpen} /></FieldIcon>
                        {districtOpen && form.provinceState && (
                          <DropdownList items={filteredDistricts} loading={false}
                            searchValue={districtSearch} onSearchChange={setDistrictSearch}
                            placeholder="Search district…" onSelect={handleDistrictSelect} />
                        )}
                      </div>
                    ) : (
                ...
                <div>
                  ...
                  <div ref={cityDropdownRef} className="relative">
                    <button type="button"
                      onClick={() => {
                        const canOpen = form.provinceState && (form.country !== SRI_LANKA_NAME || form.district);
                        if (canOpen) setCityOpen((p) => !p);
                      }}
                      className={`${dropdownBtnClass} ${cityOpen ? "border-sky-400 bg-white ring-4 ring-sky-100" : ""} ${
                        !form.provinceState || (form.country === SRI_LANKA_NAME && !form.district) ? "cursor-not-allowed opacity-50" : ""
                      }`}>
                      <span className={form.city ? "text-sky-900" : "text-sky-300"}>{form.city || "Select city"}</span>
                    </button>
                    <FieldIcon><ChevronIcon open={cityOpen} /></FieldIcon>
                    {cityOpen && form.provinceState && (form.country !== SRI_LANKA_NAME || form.district) && (
                      <DropdownList items={filteredCities} loading={false}
                        searchValue={citySearch} onSearchChange={setCitySearch}
                        placeholder="Search city…" onSelect={handleCitySelect} />
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="px-8 pt-6 pb-8">
              <div className={`${sectionHeadingClass} mb-5`}>
                <span className="flex items-center justify-center w-5 h-5 rounded-full bg-sky-100 text-[10px] font-bold text-sky-600 border border-sky-200">3</span>
                Security
                <div className="flex-1 h-px bg-sky-100" />
              </div>
              ...
              <div className="mb-5 flex items-start gap-2.5 rounded-xl border border-sky-100 bg-sky-50/60 px-4 py-3">
                ...
                <p className="text-[11.5px] leading-5 text-sky-600">
                  Password must be <span className="font-semibold">6–12 characters</span> and include at least one uppercase letter, lowercase letter, number, and special character.
                </p>
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input type={showPassword ? "text" : "password"} name="password"
                      value={form.password} onChange={handleChange} required placeholder="Password" className={fieldClass} />
                ...
                <div>
                  ...
                  <div className="relative">
                    <input type={showConfirmPassword ? "text" : "password"} name="confirmPassword"
                      value={form.confirmPassword} onChange={handleChange} required placeholder="Re-enter password" className={fieldClass} />
