  ...
  return (
    <div ref={ref}
      ...
      <div className="bg-gradient-to-br from-sky-500 to-blue-400 bg-clip-text text-[2.2rem] font-extrabold leading-none text-transparent">
        {inView ? num : 0}{hasPlus ? "+" : ""}
      </div>

      <div className="mt-1.5 text-[13px] font-semibold text-sky-900">{title}</div>
      <div className="mt-0.5 text-[11px] tracking-[0.2px] text-sky-500">{hint}</div>
    </div>
  );
}
...
const STATS = [
  { icon: icons.map,     title: "Facilities Tracked",  value: "1200+", hint: "Across all districts",       delayClass: "[animation-delay:0ms]"   },
  { icon: icons.users,   title: "Active Users",         value: "8400+", hint: "Community reporters",        delayClass: "[animation-delay:80ms]"  },
  { icon: icons.check,   title: "Issues Resolved",      value: "5600+", hint: "Verified & closed",          delayClass: "[animation-delay:160ms]" },
  { icon: icons.reports, title: "Reports Submitted",    value: "9200+", hint: "This year alone",            delayClass: "[animation-delay:240ms]" },
];

const FEATURES = [
  { icon: icons.map,   title: "Restroom Finder",        desc: "Locate nearby public restrooms with real-time availability, cleanliness ratings, and accessibility info.",              delayClass: "[transition-delay:0ms]"   },
  { icon: icons.bell,  title: "Instant Reporting",      desc: "Report water leakage, blocked toilets, or damaged facilities in seconds. Our team is notified immediately.",           delayClass: "[transition-delay:80ms]"  },
  { icon: icons.check, title: "Issue Tracking",         desc: "Follow the status of every report you submit — from open to in-progress to resolved — all in one place.",             delayClass: "[transition-delay:160ms]" },
  { icon: icons.lock,  title: "Secure & Private",       desc: "Your data is protected with role-based access. Only verified staff and administrators can act on reports.",            delayClass: "[transition-delay:240ms]" },
  { icon: icons.chart, title: "Impact Dashboard",       desc: "See live stats on issues resolved, facilities improved, and community contributions across every district.",           delayClass: "[transition-delay:320ms]" },
  { icon: icons.globe, title: "Community-Driven",       desc: "Built on verified community data that helps authorities and NGOs allocate resources where they create most impact.",   delayClass: "[transition-delay:400ms]" },
];
  ...
  return (
    <>
      ...
      <div className="min-h-screen overflow-x-hidden bg-gradient-to-br from-sky-50 via-sky-100 to-blue-200">
        ...
        {/* ── HERO ── */}
        <section className="relative mx-auto flex max-w-5xl flex-col items-center px-6 pb-16 pt-24 text-center">
          ...
          {/* Icon */}
          <div className={`mb-6 flex h-20 w-20 items-center justify-center rounded-2xl border border-sky-200 bg-sky-100/80 ${!reducedMotion ? "animate-[hmFadeDown_0.5s_0.05s_both]" : ""}`}>
            <RestroomIcon />
          </div>

          <h1 className={`relative z-[1] max-w-[820px] text-[clamp(2.4rem,6vw,4rem)] font-bold leading-[1.1] tracking-tight text-sky-900 ${!reducedMotion ? "animate-[hmFadeDown_0.55s_0.15s_both]" : ""}`}>
            Access to{" "}
            <span className="bg-gradient-to-br from-sky-400 via-sky-500 to-blue-500 bg-clip-text text-transparent">
              CWAS
            </span>{" "}
            is a
          ...
          <p className={`relative z-[1] mt-6 max-w-[520px] text-[15px] leading-[1.75] text-sky-700 ${!reducedMotion ? "animate-[hmFadeDown_0.55s_0.25s_both]" : ""}`}>
          ...
          {/* CTA buttons */}
          <div className={`mt-8 flex flex-wrap items-center justify-center gap-3 ${!reducedMotion ? "animate-[hmFadeUp_0.55s_0.35s_both]" : ""}`}>
            <Link to="/rest-rooms"
          ...
          {/* Trust pills */}
          <div className={`mt-8 flex flex-wrap justify-center gap-2 ${!reducedMotion ? "animate-[hmFadeUp_0.55s_0.45s_both]" : ""}`}>
            {["1,200+ Facilities", "8,400+ Users", "5,600+ Issues Resolved", "Sri Lanka Wide"].map((tag) => (
              <span key={tag} className="rounded-full border border-sky-200/80 bg-white/60 px-4 py-1.5 text-xs font-semibold text-sky-600 backdrop-blur-sm">
                {tag}
              </span>
            ))}
          </div>
          ...
          <div className="relative overflow-hidden rounded-3xl border border-sky-200/70 bg-white/75 px-8 py-12 shadow-xl shadow-sky-100/40 backdrop-blur-xl md:px-12">
            ...
            <div className="relative z-[1]">
              <span className="inline-flex items-center gap-1.5 rounded-full border border-sky-200 bg-sky-50 px-3 py-1 text-[11.5px] font-semibold tracking-[0.4px] text-sky-600">
                <svg className="h-3 w-3 text-sky-400" viewBox="0 0 12 12" fill="currentColor"><circle cx="6" cy="6" r="5" /></svg>
                Why it matters
              </span>
              ...
              <div className="my-8 flex items-center gap-4">
                ...
                <span className="text-[11px] text-sky-300 uppercase tracking-widest">key benefits</span>
                <div className="h-px flex-1 bg-sky-100" />
              </div>

              <div className="grid gap-4 sm:grid-cols-3">
                {[
                  { icon: icons.check, title: "Better Hygiene",     desc: "Supports cleaner public facilities and promotes healthier public spaces for everyone." },
                  { icon: icons.bell,  title: "Faster Response",    desc: "Issues reach responsible staff quickly for faster action and maintenance." },
                  { icon: icons.chart, title: "Better Management",  desc: "Track problems, assign work, and improve sanitation service quality with ease." },
                ].map(({ icon, title, desc }) => (
                  <div key={title} className="rounded-2xl border border-sky-100 bg-sky-50/60 p-5 transition-all duration-200 hover:-translate-y-0.5">
                    <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-xl border border-sky-200 bg-white/80 text-sky-600">
                      {icon}
                    </div>
                    <p className="text-[13.5px] font-semibold text-sky-900">{title}</p>
                    <p className="mt-1.5 text-[12px] leading-6 text-sky-600">{desc}</p>
                  </div>
                ))}
              </div>
            </div>
